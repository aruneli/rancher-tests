from tests.validation.cattlevalidationtest.core_target.common_fixtures import *  # NOQA
import pytest
import pickle
...
class TestRancherComposeLB:
    @pytest.mark.create
    def create_rancher_compose_lb_service(self, super_client, client):
        ...
        serviceobject['TestRancherComposeLB'] = "TARGET LB OBJECT"
        print("************************ CREATED LB OBJECT with TARGET API********************************************",
              serviceobject)
    ...
    def test_rancher_compose_lb_service(self, super_client, client):
        ...
        print("\n \n VALIDATING LB OBJECT WITH TARGET API SERVER VERSION \n\n")
        print serviceobject
        pkl_file.close()
        ...
        print(
        "\n************************ VALIDATED BASE LB OBJECT with TARGET API*******************************************\n")
        # delete_all(client, [env, rancher_env])
        print(
        "\n************************ DELETED BASE LB OBJECT DICT *****************************************************************\n")


class TestRancherComposeDNS:
    @pytest.mark.create
    def create_rancher_compose_dns_service(self, super_client, client):
        ...
        #                                   scale)
        global serviceobject
        serviceobject['TestRancherComposeDNS'] = "TARGET DNS OBJECT"
        print("************************ CREATED DNS OBJECT with TARGET API********************************************",
              serviceobject)
        output = open('myfile.pkl', 'wb')
        pickle.dump(serviceobject, output)
        output.close()
    ...
    @pytest.mark.validate_created
    def test_rancher_compose_dns_service(self, super_client, client, create=True or False):

        if create == True:
            self.create_rancher_compose_dns_service(super_client, client)
        pkl_file = open('myfile.pkl', 'rb')
        serviceobject = pickle.load(pkl_file)
        print("\n \n VALIDATING DNS OBJECT WITH TARGET API SERVER VERSION \n\n")
        print serviceobject
        pkl_file.close()

        if (serviceobject["TestRancherComposeDNS"] == ""):
            # This condition is true in case of fresh validation test
            # service, env = create_rancher_compose_service(super_client, client)
            # print ("service ================ ", service)
            # print ("env ============== ", env)
            print ("New Service created")
            print ("New env created")
        else:
            service = serviceobject["TestRancherComposeDNS"]
            print service
            ...
            #         assert inspect["Config"]["CpuShares"] == cpu_shares
        print(
        "\n************************ VALIDATED BASE DNS OBJECT with TARGET API*******************************************\n")
        # delete_all(client, [env, rancher_env])
        print(
        "\n************************ DELETED BASE DNS OBJECT DICT *****************************************************************\n")
